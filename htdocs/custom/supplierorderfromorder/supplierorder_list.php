...
if(isset($_POST['confirmmassaction'])){		
	...
	foreach($toselect as $k=>$v)
	{
		...
		$productFournisseurPriceId = GETPOST('suplier'.$commandedetID);
		$sql = '';
		$sql = 'SELECT ';		
		$sql.= 'fk_soc';		
		$sql.= " FROM ".MAIN_DB_PREFIX.'product_fournisseur_price';
		$sql.= " WHERE rowid = ".$productFournisseurPriceId;
		$resql=$db->query($sql);
		$socobj = $db->fetch_object($resql);
		...
		$sql.= " WHERE rowid = ".$commandedetID;
		$resql=$db->query($sql);
		$obj = $db->fetch_object($resql);		
		
		$commandeFournisseurArray[$socid][] = array("fk_product"=>$obj->fk_product,"description"=>'yyyy', "qty"=>$qtyToBeOrdered, "commandedetID" => $commandedetID);
	}
	...
	foreach($commandeFournisseurArray as $key=>$val)
	{
		...
		{				
			...
			$qtytosearch=$v[qty]; 	   // Just to see if a price exists for the quantity. Not used to found vat.
			//$qtytosearch=-1;	       // We force qty to -1 to be sure to find if a supplier price exist
			$idprod=$productsupplier->get_buyprice($productFournisseurPriceId, $qtytosearch);					
			...
			$tva_tx	= get_default_tva($object->thirdparty, $mysoc, $productsupplier->id, $v[fk_product]);

			$tva_npr = get_default_npr($object->thirdparty, $mysoc, $productsupplier->id, $v[fk_product]);
	        ...
	        $sql = 'INSERT INTO '.MAIN_DB_PREFIX.'supplier_order_from_order';
	        $sql.= " (fk_commandedet_id, fk_fournisseur_id";       
	        $sql.= ")";
	        $sql.= " VALUES (".$v[commandedetID]."," . $id;      
	        $sql.= ")";
	        
	        $resql=$db->query($sql);
		}
	}
	setEventMessages('Supplier Order created Successfully', null);
...
$sql.= 't.qty as toqty, t.rowid as t_rowid, so.rowid as so_rowid';
...
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."supplier_order_from_order as so on (t.rowid = so.fk_commandedet_id)";
...
{	
	...
	// Action column
	print '<td class="nowrap" align="center">';
	if ($massactionbutton || $massaction)   // If we are in select mode (massactionbutton defined) or if we have already selected and sent an action ($massaction) defined so_rowid
	{
		if(empty($obj->so_rowid)){
			$selected=0;
