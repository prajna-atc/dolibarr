...
/**
 *   	\file       submenu_card.php
 *		\ingroup    mymodule
 *		\brief      Page to create/edit/view submenu
 */
...
dol_include_once('/mymodule/lib/submenu.lib.php');
...
$object=new SubMenu($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->mymodule->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('submenucard'));     // Note that conf->hooks_modules contains array
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/mymodule/submenu_list.php',1);
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("SubMenu")));
...
{
	print load_fiche_titre($langs->trans("SubMenu"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = submenuPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("SubMenu"), -1, 'submenu@mymodule');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteSubMenu'), $langs->trans('ConfirmDeleteSubMenu'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/mymodule/submenu_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('submenu'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/mymodule/submenu_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'submenu', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
