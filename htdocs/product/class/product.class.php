...
class Product extends CommonObject
{
	...
	function fetch($id='', $ref='', $ref_ext='', $ignore_expression=0)
	{
		...
		if ( $resql )
		{
			if ($this->db->num_rows($resql) > 0)
			{
                ...
                {
					$sql = "SELECT profit_margin_rate, rebate";					
					$sql.= " FROM ".MAIN_DB_PREFIX."custom_product_rates";
					$sql.= " WHERE fk_prodid = ".$this->id;
					$resql = $this->db->query($sql);
					if ($resql)
					{
						$result = $this->db->fetch_array($resql);
						// Price by quantity
						$this->profit_margin_rate = $result["profit_margin_rate"];
						$this->rebate = $result["rebate"];
						// Récuperation de la liste des prix selon qty si flag positionné						
					}
					else
					{
    ...
    function test($idprod,$CustomerId)
    {
    	/*
    	...
    	$sql = "SELECT * FROM ".MAIN_DB_PREFIX."commandedet CMDET LEFT JOIN ".MAIN_DB_PREFIX."commande CM ON CM.rowid = CMDET.fk_commande LEFT JOIN ".MAIN_DB_PREFIX."product PD ON CMDET.fk_product = PD.rowid  WHERE CM.fk_soc = ".(int) $CustomerId ." AND CMDET.fk_product = ".(int) $idprod;   	
    	$result = $this->db->query($sql);
    	if ($result)
        {
        	if ($this->db->num_rows($result))
