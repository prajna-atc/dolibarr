...
class pdf_muscadet extends ModelePDFSuppliersOrders
{
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$printShipToAddress = '';
			$printBillToAddress = '';
			$SupplierId = $object->thirdparty->id;			
			$fournisseurId = $object->id;	
			//print $factureId.'$factureId'; exit;		
			$fournisseur = new CommandeFournisseurLigne($this->db);
			$ShipToAddress = $fournisseur->getShipToAddress($SupplierId,$fournisseurId);
			
			$BillToAddress = $fournisseur->getBillToAddress($SupplierId,$fournisseurId);
			//print_r($ShipToAddress); exit;
			if($ShipToAddress){
				$printShipToAddress=pdf_suppliershipto_address($outputlangs,$ShipToAddress,$this->emetteur,$object->thirdparty);
			}
			
			if($BillToAddress){
				$printBillToAddress=pdf_supplierbillto_address($outputlangs,$BillToAddress,$this->emetteur,$object->thirdparty);
			}
