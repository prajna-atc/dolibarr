...
class pdf_crabe extends ModelePDFFactures
{
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			//10******94******190******130******0******0
			/*$this->printRect($pdf,10,238, 105, 46, 0, 0);

			$pdf->SetXY($this->marge_gauche, $posy);
			$titreDec = 'Declaration';
			$pdf->MultiCell(105, 5, $titreDec, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+4);
			$line1 = '1. All disputes are subject to Bangalore juridiction only.';
			$pdf->MultiCell(105, 5, $line1, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+8);
			$line2 = '2. Certified that the prices charged do not exceed Publishers Prices.';
			$pdf->MultiCell(105, 5, $line2, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+12);
			$line3 = '3. Certified that the foreign prices have been converted to INR at the approved';
			$pdf->MultiCell(105, 5, $line3, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+15);
			$line31 = 'GOC Rates.';
			$pdf->MultiCell(105, 5, $line31, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+19);
			$line4 = '4. All goods are delivered entirely at buyer\'s risk. Seller is not responsible for any';
			$pdf->MultiCell(105,5, $line4, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+22);
			$line41 = 'damage/ shortage seen after delivery or dispatch from their godowns.';
			$pdf->MultiCell(105,5, $line41, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+26);
			$line5 = '5. Complaints will be entertained if filed within three days of receipt of goods.';
			$pdf->MultiCell(105, 5, $line5, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+30);
			$line6 = '6. Amount of this bill is payable in full at Bangalore and all cheques/Drafts are';
			$pdf->MultiCell(105, 5, $line6, 0, 'L');

			$pdf->SetXY($this->marge_gauche, $posy+33);
			$line61 = 'subject to realisation.';
			$pdf->MultiCell(105, 5, $line61, 0, 'L');


			$pdf->SetXY($this->marge_gauche, $posy+37);
			$line7 = '7. Goods once sold cannot be taken back or exchanged under any circumstances'; 
			$pdf->MultiCell(105, 5, $line7, 0, 'L'); 

			$pdf->SetXY($this->marge_gauche, $posy+41);
			$line7 = '8. Interest @ 18% p.a is chargeable if this bill is not paid within 45 days.'; 
			$pdf->MultiCell(105, 5, $line7, 0, 'L'); */
		}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		// Output Rect
		//print ($this->marge_gauche.'******'.$tab_top.'******'.($this->page_largeur-$this->marge_gauche-$this->marge_droite).'******'.$tab_height.'******'.$hidetop.'******'.$hidebottom);
		//exit;
...
//********************second   
		...
		$finalPosition = $beforeAddressStart+$hautcadre+15;
		//$pdf->SetTextColor(0,0,0);
		//$current_y = $pdf->getY();
		//$pdf->SetXY($posx+2,$finalPosition);
				//$pdf->MultiCell(10, 4, 'hiiiiii', 0, 'L');
		//print $pheadIntialHeight; exit;
		//print $beforeAddressStart; exit;
		return $top_shift+10;
