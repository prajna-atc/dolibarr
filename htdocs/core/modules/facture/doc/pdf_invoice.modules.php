...
class pdf_invoice extends ModelePDFFactures
{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$pdf->setTopMargin($tab_top_newpage+60);
					...
					while ($pagenb < $pageposafter)
					{
						...
						{
							$this->_tableau($pdf, $tab_top_newpage+60, $this->page_hauteur - $tab_top_newpage - $heightforfooter -60, 0, $outputlangs, 1, 1, $object->multicurrency_code);
						}
			...
			//$this->_custom_info($pdf, $object, 0, $outputlangs);
				...
				{
					$this->_tableau($pdf, $tab_top_newpage+60, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter -60, 0, $outputlangs, 1, 0, $object->multicurrency_code);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}
				$posyForAmountWords = 0;
				// Affiche zone infos
				$posy=$this->_tableau_info($pdf, $object, $bottomlasttab, $outputlangs);
				//Added by Prajna
				$posyForAmountWords = $posy;
				// Affiche zone totaux
				$posy=$this->_tableau_tot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs,$totalquantity);
				...
				if (($deja_regle || $amount_credit_notes_included || $amount_deposits_included) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS))
				{
					...
					//Added by Prajna Start
				$total_ttc = ($conf->multicurrency->enabled && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc;
				$amountInNumbers = round($total_ttc);		
				//print $amountInNumbers; exit;		
				$totalInWords = $this->convertNumberToWordsForIndia($amountInNumbers);				
			$InWordsINR = 'In Words('.$object->multicurrency_code.'):';
				$pdf->SetFont('','B',8);
				$pdf->SetXY(10, $posyForAmountWords+6);
				$pdf->MultiCell(100, 3, $InWordsINR, 0, 'L', 0);

				$pdf->SetXY(49, $posyForAmountWords+6);
				$pdf->MultiCell(($pdf->GetStringWidth($totalInWords) + 3), 3, $totalInWords, 0, 'L', 0);
				//Added by Prajna end
	...
	/*function _custom_info(&$pdf, $object, $posy, $outputlangs)
...
$pdf->MultiCell(60, 8, 'Mon textefgffffffffffffff', 0, 'L'); 
	 ...
	 */
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs,$totalquantity)
	{
		global $conf,$mysoc;
	...
	/* By gauthami start */
		...
		$pdf->SetFillColor(255,255,255);
		$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($col2x-$col1x, $tab2_hl,'Total Quantity', 0, 'L', 1);

		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, $totalquantity, 0, 'R', 1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$textref=$outputlangs->transnoentities("Ref")." : EXP/" . $outputlangs->convToOutputCharset($object->ref);
		...
		if ($showaddress)
		{
			...
			$hautcadre=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 38 : 50;
			...
			//print '<pre>';
			//print_r($object->thirdparty->name); exit;
...
//********************first   
  	...
  	// Show recipient
			...
			$posy=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 42;
			$posy+=$top_shift;
			$posx=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 92;
		//	$posx=$this->page_largeur-$this->marge_droite-$widthrecbox;
			...
			if(! empty($printBillToAddress)){
				...
				$bllToName = '';
				(!empty($BillToAddress->firstname)? $bllToName .= $BillToAddress->firstname.' ':'');
				(!empty($BillToAddress->lastname)? $bllToName .= $BillToAddress->lastname:'');
				...
				$pdf->SetXY($posx+2,$posy);
				$pdf->SetFont('','B', $default_font_size);
				$pdf->MultiCell($widthrecbox, 2, $object->thirdparty->name, 0, 'L');

				// Show recipient name
				$pdf->SetXY($posx+2,$posy+4);
				$pdf->SetFont('','B', $default_font_size);
				$pdf->MultiCell($widthrecbox, 2, $bllToName, 0, 'L');
...
//********************second   
			...
			$posy=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 42;
			$posy+=$top_shift;
			$posx=!empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 145.5;
		//	$posx=$this->page_largeur-$this->marge_droite-$widthrecbox;
			if (! empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx=$this->marge_gauche;

			if(! empty($printShipToAddress)){
				$shipToName = '';
				(!empty($ShipToAddress->firstname)? $shipToName .= $ShipToAddress->firstname.' ':'');
				(!empty($ShipToAddress->lastname)? $shipToName .= $ShipToAddress->lastname:'');
				...
				$pdf->SetXY($posx+2,$posy+4);
				$pdf->SetFont('','B', $default_font_size);
				$pdf->MultiCell($widthrecbox, 2, $shipToName, 0, 'L');
		...
		$pdf->SetXY($posx-140,$posy+210);
		$this->printRect($pdf,$posx-135,$posy+50,190,30);
		...
		//exit;
		$pdf->line($posx+5,$posy-150, $posx+195, $posy-150);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-158);
		$pdf->MultiCell(40,5, 'Pre- Carrier:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-154);
		$pdf->MultiCell(40,5, $object->array_options['options_pre_carrier'], 0, 'L');

		$pdf->line($posx+50,$posy-160, $posx+50, $posy-140);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+50,$posy-158);
		$pdf->MultiCell(50, 5, 'Place of receipt by Pre-carrier:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+50,$posy-154);
		$pdf->MultiCell(50, 5, $object->array_options['options_placeofreceipt'], 0, 'L');
	
		$pdf->line($posx+105,$posy-160, $posx+105, $posy-130);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-158);
		$pdf->MultiCell(40, 5, 'Vessel/Flight No:	',0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-154);
		$pdf->MultiCell(40, 5, $object->array_options['options_vessel_flightno'],0, 'L');

		$pdf->line($posx+155,$posy-160, $posx+155, $posy-140);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+155,$posy-158);
		$pdf->MultiCell(40, 5, 'Port of Loading:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+155,$posy-154);
		$pdf->MultiCell(40, 5, $object->array_options['options_port_of_loading'], 0, 'L');
	

		$pdf->line($posx+5,$posy-140, $posx+195, $posy-140);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-148);
		$pdf->MultiCell(40, 5, 'Port of Discharge:	', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-144);
		$pdf->MultiCell(40, 5, $object->array_options['options_port_of_discharge'], 0, 'L');


		
		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+50,$posy-148);
		$pdf->MultiCell(50, 5, 'Marks & Nos:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+50,$posy-144);
		$pdf->MultiCell(50, 5, $object->array_options['options_marks_no'], 0, 'L');

		//$pdf->line($posx+105,$posy-160, $posx+105, $posy-120);
		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-148);
		$pdf->MultiCell(40, 5, 'No. & Kinds of Pkgs:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-144);
		$pdf->MultiCell(40, 5, $object->array_options['options_no_kind_of_pkgs'], 0, 'L');


		$final=explode(':',$conf->global->MAIN_INFO_SOCIETE_COUNTRY);
		
		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+155,$posy-148);
		$pdf->MultiCell(40, 5, 'Description of goods:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+155,$posy-144);
		$pdf->MultiCell(40, 5, $object->array_options['description_of_goods'], 0, 'L');

		$pdf->line($posx+5,$posy-130, $posx+5, $posy-130);

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-138);
		$pdf->MultiCell(60, 5, 'Country of Origin:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+5,$posy-134);
		$pdf->MultiCell(60, 5, $final[2], 0, 'L');

		$pdf->SetFont('','B', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-138);
		$pdf->MultiCell(60, 5, 'Country of final Destination:', 0, 'L');

		$pdf->SetFont('','', $default_font_size - 1);
		$pdf->SetXY($posx+105,$posy-134);
		$pdf->MultiCell(60, 5, $object->thirdparty->country, 0, 'L');
...
function Show_Amount_In_Words($num) {
	...
	$check = (int) $num;
	 	if($check !=0){
	 		$returnVal = $str.' Rupees '.$this->forDecimal($fo1);
	        //return $str.' Rupees '.$this->forDecimal($fo1).' Only';
	    }else{
	    	$returnVal = $this->forDecimal($fo1);
	       	//return $this->forDecimal($fo1).' Only';
	    }
	    if(!empty($returnVal)){ return $returnVal.' Only';}
	    return 'NILL';
