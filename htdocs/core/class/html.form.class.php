...
class Form
{
	...
	/**
	*      Return list of suppliers for a product.
	*      Constant MAIN_DEFAULT_PAYMENT_TERM_ID can used to set default value but scope is all application, probably not what you want.
	*      See instead to force the default value by the caller.
	*
	*      @param	int $selectSupplier	Id of product
	*	@return	void
	*/
	function selectSupplier($selectSupplier,$commandeId)
	{


	global $db,$langs;

	$sql = "SELECT ps.fk_soc,ps.price,ps.quantity,ps.rowid,ls.nom";
	$sql.= " FROM llx_product_fournisseur_price ps left join llx_societe ls on ps.fk_soc=ls.rowid";
	$sql.= " where ps.fk_product=".$selectSupplier;
	$sql.= " ORDER  BY ps.rowid DESC";

	//$sql.= " GROUP BY ps.fk_soc";
	$resql = $db->query($sql);
	$out= '<select name="suplier'.$commandeId.'">';

	while ($obj = $db->fetch_object($resql))
	{

	$out.= '<option  value="'.$obj->rowid.'">'.$obj->nom.'-'.$obj->price.'</option>';
	}
