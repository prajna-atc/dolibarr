...
function print_eldy_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0,$mode='')
{
	...
	for($i = 0; $i < $num; $i++)
	{
		...
		//echo $newTabMenu[$i]['titre'];
		if($newTabMenu[$i]['titre']=='SupplierOrderFromOrder')
		{
				$menu->add($shorturl, $newTabMenu[$i]['titre'], 0, 0, ($newTabMenu[$i]['target']?$newTabMenu[$i]['target']:$atarget), ($newTabMenu[$i]['mainmenu']?$newTabMenu[$i]['mainmenu']:$newTabMenu[$i]['rowid']), ($newTabMenu[$i]['leftmenu']?$newTabMenu[$i]['leftmenu']:''), $newTabMenu[$i]['position'], $id, $idsel, $classname);
		}
		else
		{
