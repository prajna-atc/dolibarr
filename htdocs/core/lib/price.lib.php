 ...
 */
function calcul_price_total($qty, $pu, $remise_percent_ligne, $txtva, $uselocaltax1_rate, $uselocaltax2_rate, $remise_percent_global, $price_base_type, $info_bits, $type, $seller = '', $localtaxes_array='', $progress=100, $multicurrency_tx=1, $pu_devise=0,$fk_product=0)
{
	...
	//print $pu.'$pu,'.$qty.'$qty,'.$progress.'$progress'; exit;
	//Added by Prajna
	if (!empty($conf->global->CUSTOM_PRODUCT_PROFITMARGIN) && !empty($conf->global->CUSTOM_PRODUCT_REBATE))
    {
        ...
        $sql1.= " WHERE fk_prodid = ".$fk_product;   
        $resql1 = $db->query($sql1);
        if ($resql1)
        {
            $result1 = $db->fetch_array($resql1);            
            $num1 = $db->num_rows($result1);            
            // Price by quantity
            if($num1 == 1){
                $marginRate = $result1["profit_margin_rate"];
                $rebate = $result1["rebate"];
            }else{
                // Récuperation de la liste des prix selon qty si flag positionné 
                $marginRate = $conf->global->CUSTOM_PRODUCT_PROFITMARGIN;
                $rebate = $conf->global->CUSTOM_PRODUCT_REBATE;
            }
	...
	//print $marginRate.'$marginRate,'. $rebate.' $rebate'; exit;
	$temp_pu = (( $pu * $rebate ) + ($marginRate / 100));
	//print '$pu'.$pu; exit;
	//print '$temp_pu'.$temp_pu; exit;
	//print 'temppu-'.$temp_pu.'pu-'. $pu; exit;
	if (! empty($conf->global->CUSTOM_PRODUCT_PROFITMARGIN))
    {
    	$tot_sans_remise = $temp_pu * $qty * $progress / 100;
