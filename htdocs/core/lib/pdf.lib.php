...
function pdf_pagefoot(&$pdf,$outputlangs,$paramfreetext,$fromcompany,$marge_basse,$marge_gauche,$page_hauteur,$object,$showdetails=0,$hidefreetext=0)
{
	...
	if ($line)	// Free text
	{
		...
		{
            $pdf->writeHTMLCell($pdf->page_largeur - $pdf->margin_left - $pdf->margin_right, $freetextheight, $dims['lm'], $dims['hk']-$marginwithfooter, dol_htmlentitiesbr($line, 1, 'UTF-8', 0),1);
		}
		$posy-=$freetextheight;
	}
...
function pdf_suppliershipto_address($outputlangs,$address,$sourcecompany,$targetcompany)
{
	global $conf;
	$stringaddress = '';
	$withCountry = 0;
...
function pdf_supplierbillto_address($outputlangs,$address,$sourcecompany,$targetcompany)
{
	global $conf;
	$stringaddress = '';
	$withCountry = 0;
