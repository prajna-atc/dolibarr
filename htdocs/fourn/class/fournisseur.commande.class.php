...
class CommandeFournisseur extends CommonOrder
{
    ...
    function fetch_lines($only_product=0)
    {
    	...
    	if ($result)
    	{
    		...
    		while ($i < $num)
    		{
                ...
                if (!empty($conf->global->CUSTOM_PRODUCT_PROFITMARGIN) && !empty($conf->global->CUSTOM_PRODUCT_REBATE))
                {
                    $sql1 = "SELECT profit_margin_rate, rebate";                 
                    $sql1.= " FROM ".MAIN_DB_PREFIX."custom_product_rates";
                    $sql1.= " WHERE fk_prodid = ".$objp->fk_product;                
                    $resql1 = $this->db->query($sql1);
                    if ($resql1)
                    {
                        
                        $result1 = $this->db->fetch_array($resql1);
                        $num1 = $this->db->num_rows($result1);
                        // Price by quantity
                        if($num1 == 1){
                            $line->profit_margin_rate = $result1["profit_margin_rate"];
                            $line->rebate = $result1["rebate"];
                        }else{
                            // Récuperation de la liste des prix selon qty si flag positionné 
                            $line->profit_margin_rate = $conf->global->CUSTOM_PRODUCT_PROFITMARGIN;
                            $line->rebate = $conf->global->CUSTOM_PRODUCT_REBATE;
                        }
	...
	public function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1=0.0, $txlocaltax2=0.0, $fk_product=0, $fk_prod_fourn_price=0, $ref_supplier='', $remise_percent=0.0, $price_base_type='HT', $pu_ttc=0.0, $type=0, $info_bits=0, $notrigger=false, $date_start=null, $date_end=null, $array_options=0, $fk_unit=null, $pu_ht_devise=0, $origin='', $origin_id=0)
    {
        ...
        if ($this->statut == self::STATUS_DRAFT)
        {
            ...
            $tabprice = calcul_price_total($qty, $pu, $remise_percent, $txtva, $txlocaltax1, $txlocaltax2, 0, $price_base_type, $info_bits, $product_type, $this->thirdparty, $localtaxes_type, 100, $this->multicurrency_tx,$pu_ht_devise, $fk_product);
    ...
    public function updateline($rowid, $desc, $pu, $qty, $remise_percent, $txtva, $txlocaltax1=0, $txlocaltax2=0, $price_base_type='HT', $info_bits=0, $type=0, $notrigger=0, $date_start='', $date_end='', $array_options=0, $fk_unit=null, $pu_ht_devise=0, $ref_supplier='')
    {
        ...
        if ($this->brouillon)
        {
            ...
            $tabprice=calcul_price_total($qty, $pu, $remise_percent, $txtva, $txlocaltax1, $txlocaltax2, 0, $price_base_type, $info_bits, $type, $this->thirdparty, $localtaxes_type, 100, $this->multicurrency_tx, $pu_ht_devise,0);
            $total_ht  = $tabprice[0];
            $total_tva = $tabprice[1];
            $total_ttc = $tabprice[2];
            $total_localtax1 = $tabprice[9];
...
class CommandeFournisseurLigne extends CommonOrderLine
{
    ...
    function getShipToAddress($SupplierId,$fournisseurId)
    {       
        /*
    ...
    function getBillToAddress($SupplierId,$fournisseurId)
    {       
        /*
                $sql = "SELECT DISTINCT fk_prod_attr, a.rang
            FROM ".MAIN_DB_PREFIX."product_attribute_combination2val c2v LEFT JOIN ".MAIN_DB_PREFIX."product_attribute_combination c
            ...
            $sql = "SELECT * FROM ".MAIN_DB_PREFIX."element_contact ec LEFT JOIN ".MAIN_DB_PREFIX."socpeople c ON ec.fk_socpeople = c.rowid LEFT JOIN  ".MAIN_DB_PREFIX."c_type_contact tc ON ec.fk_c_type_contact = tc.rowid WHERE ec.element_id = ".(int) $fournisseurId." AND tc.active = 1 AND c.entity IN (1) AND tc.element = 'order_supplier' AND tc.source = 'external' AND tc.code='BILLING'"; 
        $result = $this->db->query($sql);       
        if ($result)
        {
            if ($this->db->num_rows($result))
